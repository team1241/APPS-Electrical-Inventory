...
function DropdownMenuSubTrigger({
  ...
  return (
    <MenuPrimitive.SubmenuTrigger
      ...
      data-inset={inset}
      className={cn(
        "group/dropdown-menu-sub-trigger flex cursor-default items-center gap-2 rounded-2xl px-3 py-2 text-sm font-medium outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**:text-accent-foreground data-inset:pl-9.5 data-popup-open:bg-accent data-popup-open:text-accent-foreground data-open:bg-accent data-open:text-accent-foreground [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
      {children}
      <ChevronRightIcon className="ml-auto transition-transform duration-200 ease-out group-data-popup-open/dropdown-menu-sub-trigger:rotate-90 group-data-open/dropdown-menu-sub-trigger:rotate-90" />
    </MenuPrimitive.SubmenuTrigger>
  )
}
